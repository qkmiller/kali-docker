...
RUN apt -y update && \
    apt -y full-upgrade && \
    apt -y install less man openvpn sudo curl tmux vim nano iputils-ping git python3 \
    nmap snmp git gobuster dirbuster wordlists gcc gdb wpscan exploitdb && \
    apt -y auto-remove
